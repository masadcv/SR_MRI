...
class MINet(nn.Module):
    def __init__(self, n_resgroups, n_resblocks, n_feats):
        ...
        self.fusion_convs = nn.ModuleList(
            [nn.Conv2d(128, 64, kernel_size=1, padding=0) for i in range(nlayer)]
        )
        self.fusion_convsT1 = nn.ModuleList(
            [nn.Conv2d(64, 32, kernel_size=1, padding=0) for i in range(nlayer)]
        )
        self.fusion_convsT2 = nn.ModuleList(
            [nn.Conv2d(64, 32, kernel_size=1, padding=0) for i in range(nlayer)]
        )
        self.map_convs = nn.ModuleList(
            [nn.Conv2d(64, 1, kernel_size=3, padding=1) for i in range(nlayer)]
        )
    ...
    def forward(self, x1, x2):
        ...
        for m1, m2, fusion_conv in zip(
            self.net1.body._modules.items(),
            self.net2.body._modules.items(),
            self.fusion_convs,
        ):
            name1, midlayer1 = m1
            _, midlayer2 = m2
